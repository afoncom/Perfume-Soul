...
extension RecommendedPerfumePresenterImpl: RecommendedPerfumePresenter {
    func resolve() async {
        ...
        if let state = stateStorage.loadState(), state.profileCalculationCacheKey == profile.profileCalculationCacheKey, state.topPerfumeIDs == topIDs, state.weekKey == weekKey {
        ...
        do {
            let candidates = try await service.requestCandidates(profile: makeProfileRequest(calculation), excludedPerfumeIDs: exclusions.sorted(), limit: Self.candidateLimit).map { DailyPerfumeSummary(id: $0.id, perfumeName: $0.perfumeName, brandName: $0.brandName) }
            let old = stateStorage.loadState()?.perfumes.filter { !exclusions.contains($0.id) } ?? []
            ...
            stateStorage.saveState(RecommendedPerfumeState(perfumes: perfumes, reservePerfumes: reserve, topPerfumeIDs: topIDs, profileCalculationCacheKey: profile.profileCalculationCacheKey, weekKey: weekKey))
            await show(perfumes)
        } catch {
            await show([])
        }
    ...
    @MainActor func savePerfume(_ perfume: DailyPerfumeSummary) {
        ...
        guard var state = stateStorage.loadState() else { return }
...
private extension RecommendedPerfumePresenterImpl {
    func makeProfileRequest(_ calculation: ProfileCalculation) -> DailyPerfumeProfileRequest {
        DailyPerfumeProfileRequest(sun: calculation.natalChart.sun.sign.rawValue, moon: calculation.natalChart.moon.sign.rawValue, ascendant: calculation.natalChart.ascendant.sign.rawValue, elementBalance: DailyPerfumeElementBalanceRequest(fire: calculation.elementBalance.fire, earth: calculation.elementBalance.earth, air: calculation.elementBalance.air, water: calculation.elementBalance.water))
    }

    func show(_ perfumes: [DailyPerfumeSummary]) async {
