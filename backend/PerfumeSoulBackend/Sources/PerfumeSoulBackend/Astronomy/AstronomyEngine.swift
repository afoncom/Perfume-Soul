...
private extension AstronomyEngine {
    ...
    static func longitude(
        result: astro_angle_result_t,
        label: String
    ) throws -> Double {
        try assertSuccess(result.status, label: label)
        return normalize(result.angle)
    }
    ...
    static func ascendantLongitude(
        ...
        // In the horizontal system y points west, so east-facing vectors must have negative y.
